...
def main(page: ft.Page):
    ...
    # Чистое подключение к базе (никаких input!)
    def init_db():
        try:
    ...
    def update_messages(docs, changes, read_time):
    ...
    def show_login():
        ...
        def do_login(e):
            if u_in.value:
                ...
                if init_db(): show_chat()
                    ...
                    page.snack_bar = ft.SnackBar(ft.Text("DB ERROR: Check JSON file"))
        ...
        container.controls.append(
            ft.Container(
                content=ft.Column([
                    ft.Icon(ft.icons.LOCK_OPEN_ROUNDED, size=80, color="#00FF00"),
                    ft.Text("GHOST PRO AUTH", size=30, weight="bold", color="#00FF00"),
                    u_in, p_in,
                    ft.ElevatedButton("INITIALIZE", on_click=do_login, bgcolor="#002200", color="#00FF00", width=300, height=50)
    ...
    def show_chat():
        ...
        if st.db:
            st.db.collection("messages").order_by("ts", descending=False).limit_to_last(50).on_snapshot(update_messages)
        ...
        container.controls.append(
            ft.Column([
                ft.Container(
                    content=ft.Row([
                        ft.Text(f"USER: {st.user}", color="#00FF00", size=12),
                        ft.IconButton(ft.icons.DELETE_FOREVER, icon_color="red", visible=(st.role=="ADMIN"), on_click=admin_clear)
                    ], justify="spaceBetween"),
